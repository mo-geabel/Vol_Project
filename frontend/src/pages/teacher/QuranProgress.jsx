...
const QuranProgress = () => {
  ...
  const handleLogProgress = async (e) => {
    ...
    try {
      ...
      toast.success(`${logType} logged successfully for ${selectedEnrollment.student.name}`);
  ...
  const handleLogNotPrepared = async (enrollment, type) => {
    try {
      ...
      const label = type === 'NotPreparedHifz' ? 'Hifz' : 'Muraja';
      toast.success(`Marked ${enrollment.student.name} as Not Prepared for ${label}`);
      setShowLogModal(false);
      fetchDailyProgress(); // Refresh indicators
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Log Quranic progress and Muraja</p>
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="text-sm text-gray-500">
          Showing {filteredStudents.length} of {enrollments.length} students
        </div>
      </div>
            ...
            <div key={enrollment.id} className="card overflow-hidden hover:shadow-lg transition-shadow bg-white flex flex-col group relative">
              <div className="p-5 border-b border-gray-100">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      {enrollment.student.date_of_birth && (
                        <span className="text-[10px] bg-indigo-50 text-slate-500 px-1.5 py-0.5 rounded font-black">
                          Age: {calculateAge(enrollment.student.date_of_birth)}
                        </span>
                      )}
                ...
                <div className="flex flex-wrap gap-2">
                  {status.hasHifz && (
                    <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-1 bg-green-100 text-green-700 rounded-lg">
                      <BookOpen size={10} />
                      Hifz Signed
                    </span>
                  )}
                  {status.isNotPreparedHifz && (
                    <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-1 bg-amber-100 text-amber-700 rounded-lg border border-amber-200">
                      <AlertCircle size={10} />
                      Unprepared (H)
                    </span>
                  )}
                  {status.hasMuraja && (
                    <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-1 bg-blue-100 text-blue-700 rounded-lg">
                      <BookOpen size={10} />
                      Muraja Signed
                    </span>
                  )}
              ...
              <div className="p-4 flex flex-col gap-3">
                ...
                <button 
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2 border border-gray-200 text-gray-600 rounded-xl hover:bg-gray-50 transition-colors text-sm font-medium"
                >
      ...
      {showLogModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 bg-linear-to-r from-primary-600 to-primary-500 text-white">
              <div className="flex justify-between items-center">
                <div>
                  <h3 className="text-xl font-bold">Sign {logType}</h3>
            ...
            <form onSubmit={handleLogProgress} className="p-6 space-y-4">
              <div className="grid grid-cols-1 gap-4">
                  ...
                  >
                    ...
                    {surahs.map(s => (
                      <option key={s.surah_id} value={s.surah_id}>
                        {s.surah_id}. {s.surah_name} ({s.verse_count} verses)
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Start Verse</label>
                    <input 
                      type="number" 
                      required
                      min="1"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">End Verse</label>
                    <input 
                      type="number" 
                      required
              ...
              <div className="flex flex-col gap-3 pt-6">
                ...
                <button 
                  ...
                  className="w-full flex items-center justify-center gap-2 py-3 border border-orange-200 text-orange-600 rounded-2xl hover:bg-orange-50 transition-colors text-sm font-bold uppercase tracking-wider"
                >
                  <AlertCircle size={18} />
      ...
      {showEditStudentModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 bg-linear-to-r from-indigo-600 to-indigo-500 text-white">
              <div className="flex justify-between items-center">
                <div>
                  <h3 className="text-xl font-bold">Edit Student</h3>
                  <p className="text-indigo-100 text-sm">Update basic information</p>
                </div>
                <div className="p-3 bg-white/20 rounded-2xl">
            ...
            <form onSubmit={handleUpdateStudent} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-sm font-medium text-gray-700 mb-1 flex items-center gap-1.5">
                    <Phone size={14} className="text-primary-500" /> Contact Info
                  </label>
                  <input 
                    type="text"
              ...
              <div>
                <label className="text-sm font-medium text-gray-700 mb-1 flex items-center gap-1.5">
                  <User2 size={14} className="text-primary-500" /> Parent Details
                </label>
                <input 
                  type="text"
